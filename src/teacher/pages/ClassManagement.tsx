...
const ClassManagement = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    const [
      ...
      supabase
        .from("classes" as any)
        .select("*, curricula(title), class_enrollments(count)")
  ...
  const handleCreate = async () => {
    ...
    setSaving(false);
    if (error) {
      toast.error("فشل في إنشاء الفصل: " + error.message);
      return;
    }
  ...
  return (
    <div className="space-y-6">
      ...
      {showForm && (
        <div className="bg-card rounded-2xl border border-border p-6 space-y-4">
          <input
            ...
            placeholder="اسم الفصل (مثال: الرياضيات - الصف التاسع ب) *"
      ...
      <div className="space-y-4">
        ...
        {classes.map((c: any) => (
          <div key={c.id} className="bg-card rounded-2xl border border-border p-6 hover-lift">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    المنهج: {c.curricula?.title || "غير محدد"}
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-muted/50 rounded-2xl p-4 flex flex-col items-center justify-center border border-border">
                ...
                >
                  <Copy className="w-3 h-3" /> نسخ رمز الانضمام
                </button>
              </div>
              <div className="bg-muted/50 rounded-2xl p-4 flex flex-col items-center justify-center border border-border">
                <p className="text-2xl font-black mb-1">
                  {(c.class_enrollments as any)?.[0]?.count ?? 0}
                </p>
